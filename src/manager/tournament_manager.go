...
func (tm *TournamentManager) setTableAsRequested(clientReq request.DealerRequest) error {
	for i, _ := range tm.Tables {
		if tm.Tables[i].ID == clientReq.ID {
			tm.Tables[i].PlayersNum = clientReq.PlayersNum
			return nil
		}
	}
...
func (tm *TournamentManager) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	var clientReq request.DealerRequest
	...
	if clientReq.ID == 0 {
		...
		tm.Tables = append(tm.Tables, table.NewTable(tableID, clientReq.PlayersNum))

		// TODO: ここでクライアント側にtableIDを返す処理が必要
		fmt.Fprintf(w, "Your tableID is %v\n", tableID)
	} else if len(tm.Tables) > 0 {
		if err = tm.setTableAsRequested(clientReq); err != nil {
